...
public class GetOptionalElementsDao {
    ...
    private final String GET_OPTIONAL_ELEMENTS_BY_TYPE = "SELECT oe FROM OptionalElements oe WHERE oe.type = :type";
    ...
    public OptionalElements getByType(String type, EntityManager em) {
        try {
            return em.createQuery(GET_OPTIONAL_ELEMENTS_BY_TYPE, OptionalElements.class).setParameter("type", type).getSingleResult();
        } catch(Exception e) {
